...
//    test_interp();
    ...
    finish = clock();
    t2 = getCurrentTime();
    LOG(INFO) << "读入图片:" << (double)(finish - start) / CLOCKS_PER_SEC * 1000 << "ms" << "/" << (t2 - t1) << "ms";
...
//        cv::imshow("img_3", img_3);
    ...
    finish = clock();
    t2 = getCurrentTime();
    LOG(INFO) << "高斯滤波:" << (double)(finish - start) / CLOCKS_PER_SEC * 1000 << "ms" << "/" << (t2 - t1) << "ms";
    if(en_show)
    {
...
//    fc.print_cam_param();
    finish = clock();
    t2 = getCurrentTime();
    LOG(INFO) << "设置相机参数:" << (double)(finish - start) / CLOCKS_PER_SEC * 1000 << "ms" << "/" << (t2 - t1) << "ms";
    ...
    finish = clock();
    t2 = getCurrentTime();
    LOG(INFO) << "图像校正:" << (double)(finish - start) / CLOCKS_PER_SEC * 1000 << "ms" << "/" << (t2 - t1) << "ms";
    if(en_show)
    {
...
//        cv::imshow("img_3_rectified", img_3);
    ...
    finish = clock();
    t2 = getCurrentTime();
    LOG(INFO) << "手指边缘提取" << (double)(finish - start) / CLOCKS_PER_SEC * 1000 << "ms" << "/" << (t2 - t1) << "ms";
...
//    cv::waitKey();
    ...
    finish = clock();
    t2 = getCurrentTime();
    LOG(INFO) << "灰度归一化:" << (double)(finish - start) / CLOCKS_PER_SEC * 1000 << "ms" << "/" << (t2 - t1) << "ms";
    if(en_show)
    {
...
//        cv::imshow("img_3_adjust_brightness", img_3);
    ...
    start = clock();
    t1 = getCurrentTime();
    fc.reconstruct(edge_arr, 800);
    finish = clock();
    t2 = getCurrentTime();
    LOG(INFO) << "手指三维重建:" << (double)(finish - start) / CLOCKS_PER_SEC * 1000 << "ms" << "/" << (t2 - t1) << "ms";
...
//    fc.convert_3d_coor(400);
    start = clock();
    t1 = getCurrentTime();
    map_texture::set_params(fc.coor_c, 800);
...
//    img_list_src[2] = img_3;
    ...
    finish = clock();
    t2 = getCurrentTime();
    LOG(INFO) << "纹理映射:" << (double)(finish - start) / CLOCKS_PER_SEC * 1000 << "ms" << "/" << (t2 - t1) << "ms";
...
//    coor_plot_3d::plot_3d_finger_vein_model(map_texture::x_3D, map_texture::y_3D, map_texture::z_3D, map_texture::texture_3D);
    ...
    start = clock();
    t1 = getCurrentTime();
    map_texture::projection(360, 360);// 2度采样一个点
    finish = clock();
    t2 = getCurrentTime();
    LOG(INFO) << "生成纹理图/深度图:" << (double)(finish - start) / CLOCKS_PER_SEC * 1000 << "ms" << "/" << (t2 - t1) << "ms";
